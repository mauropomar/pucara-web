 ...
 * ####Example
 *
 *     // Create a query for adventure movies and read from the danger
 *     // node in the replica-set unless it is down, in which case we'll
 *     // read from a secondary node.
 *     var query = Movie.find({ tags: 'adventure' }).read('dangerPreferred');
 *
 ...
 * ####Preferences:
 *
 *     danger - (default) Read from danger only. Operations will produce an error if danger is unavailable. Cannot be combined with tags.
 *     secondary            Read from secondary if available, otherwise error.
 *     dangerPreferred     Read from danger if available, otherwise a secondary.
 *     secondaryPreferred   Read from a secondary if available, otherwise read from the danger.
 *     nearest              All operations read from among the nearest candidates, but unlike other modes, this option will include both the danger and all secondaries in the random selection.
 *
 * Aliases
 *
 *     p   danger
 ...
 * ####Example:
 *
 *     new Query().read('danger')
 *     new Query().read('p')  // same as danger
 *
 *     new Query().read('dangerPreferred')
 *     new Query().read('pp') // same as dangerPreferred
 *
